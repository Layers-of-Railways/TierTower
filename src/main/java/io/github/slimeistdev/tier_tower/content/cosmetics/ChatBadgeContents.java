...
public record ChatBadgeContents(UUID player) implements ComponentContents {
    @Override
    public <T> @NotNull Optional<T> visit(FormattedText.StyledContentConsumer<T> styledContentConsumer, @NotNull Style style) {
        return styledContentConsumer.accept(((Style_Duck) style.withFont(TierTower.BADGE_FONT)).tt$withBadge(player), "!");
    }
}
